  ...
  return new Promise((resolve, reject) => {
    if (req.headers && req.headers.authorization) {
      ...
      if (parts.length === 2) {
        ...
        return reject(new sails.utils.ServiceError(400, 'Bad authorization header format', 'E_TOKEN_BAD_FORMAT'))
      }
    } else {
      ...
      if (!R.isNil(token)) {
        ...
        return reject(new sails.utils.ServiceError(401, 'No token found', 'E_TOKEN_MISSING'))
        // return reject(null)
      }
    }
  })
...
TokenService.validateToken = function validateToken (token) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (_token.exp <= _reqTime) {
      ...
      return reject(new sails.utils.ServiceError(401, 'Your token is expired.', 'E_TOKEN_EXPIRED'))
    }
    ...
    if ((_reqTime + clockTolerance) <= _token.nbf) {
      ...
      return reject(new sails.utils.ServiceError(401, 'This token is early.', 'E_TOKEN_EARLY'))
    }
    ...
    if (sails.config.jwt.audience !== _token.aud) {
      ...
      return reject(new sails.utils.ServiceError(401, 'This token cannot be accepted for this domain.', 'E_TOKEN_DOMAIN'))
    }
